...
class SanaLinearAttnProcessor2_0_SharedKVAll:
    ...
    def _self_attention_linear_sharedkv(self, attn: Attention, hidden_states: torch.Tensor,
        ...
        # (B, H, N, K) * (C, H, K, V) -> (B, C, H, V)
        proxy_output_sum = torch.einsum('b h n k, c h k v -> b c h v', phi_q_masked, S_frame_masked_k)
        raw_dot_product = proxy_output_sum / Tq  # 평균 스코어

        """
        ############헤드 차원 압축한 내적 시각화 코드
        if self.current_dump_path is not None:
            try:
                # raw_dot_product shape: (B, C, H, V)
                
                # 1. 헤드 차원(dim=2)만 평균 냅니다. (압축)
                # 결과 shape: (B, C, V) -> 배치는 살아있음!
                head_mean_dot = raw_dot_product.mean(dim=2)
                
                # 2. 데이터 펼치기
                # (B, C, V) -> (B, -1)
                B_dot = head_mean_dot.shape[0]
                flattened_dot = head_mean_dot.view(B_dot, -1).detach().float().cpu().numpy()

                data_list = []
                for i in range(B_dot):
                    values = flattened_dot[i]
                    # 샘플링
                    if len(values) > 5000:
                         values = np.random.choice(values, 5000, replace=False)
                    
                    for v in values:
                        # 배치 인덱스를 다시 살려서 저장합니다.
                        data_list.append({'Batch_Index': f'Image {i}', 'Value': v})

                df_dot = pd.DataFrame(data_list)

                # 3. 시각화
                plt.figure(figsize=(10, 5))
                # Hue를 사용하여 이미지별로 색깔을 다르게 그립니다.
                sns.kdeplot(data=df_dot, x='Value', hue='Batch_Index', fill=True, palette="Set2", alpha=0.3)
                
                plt.title(f'Head-Averaged Dot Product Distribution (Step {self.global_step_counter})')
                plt.xlabel('Mean Dot Product Value')
                plt.grid(True, alpha=0.3)
                plt.tight_layout()

                # 4. 저장 (파일명: head_mean_dot_...)
                safe_name = self.my_name.replace(".", "_")
                save_name = f"head_mean_dot_{safe_name}_step_{self.global_step_counter:03d}.png"
                ...
                print(f"[Warning] Failed to plot head mean dot product: {e}")
        ...
        S_base = torch.einsum('b c h v, c h k v -> b h k v', channel_weights, S_frame_masked_k)
       
        # z(분모) Aggregation
        frame_weights_mean = channel_weights.mean(dim=-1, keepdim=True) # (B, C, H, 1)
        z_base = torch.einsum('b c h i, c h k i -> b h k i', frame_weights_mean, z_frame_masked_k)
 
        # Self Correction
        indices = torch.arange(B, device=phi_q.device)
        self_channel_weights = channel_weights[indices, indices, :, :] # (B, H, D)
        self_channel_weights = self_channel_weights.unsqueeze(2)
 
        self_boost = 1.0
 
        S_combined = S_base + self_channel_weights * (self_boost * S_frame_full - S_frame_masked_k)
       
        self_z_weight = frame_weights_mean[indices, indices, :, :] # (B, H, 1)
        self_z_weight = self_z_weight.unsqueeze(-1)
        z_combined = z_base + self_z_weight * (self_boost * z_frame_full - z_frame_masked_k)
